...
function sendNotification($user_id, $request_id, $message, $type = 'Dashboard') {
    ...
    if ($type === 'Email') {
        ...
        if ($userResult && $userResult->num_rows > 0) {
            ...
            if (strtolower($settings['transport']) === 'smtp') {
            ...
            if (!$success) {
                error_log("Email delivery failed for notification to {$email}. Configure MAIL_FROM and MAIL_TRANSPORT or SMTP settings.");
            }
        }
    }
